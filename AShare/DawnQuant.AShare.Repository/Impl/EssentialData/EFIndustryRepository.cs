...
namespace DawnQuant.AShare.Repository.Impl.EssentialData
{
        ...
        IIndustryRepository
    {
        ...
        {
            //第一级
            Industry fIndustry = _dbContext.Entities.SingleOrDefault(p => p.Name == industry.first && p.Level == 1);
            ...
            //第二级别
            Industry sIndustry = _dbContext.Entities.SingleOrDefault(p => p.Name == industry.second && p.Level == 2);
            if (sIndustry == null)
            {
            ...
            //第三级别
            Industry tIndustry = _dbContext.Entities.SingleOrDefault(p => p.Name == industry.three && p.Level == 3);
            if (tIndustry == null)
            {
        ...
        {
            Industry industry = _dbContext.Entities.SingleOrDefault(p => p.Id == id);
            return industry == null ? null :
                 _dbContext.Entities.SingleOrDefault(p => p.Id == industry.ParentId);
        }

        public IQueryable<Industry> GetSubIndustry(int id)
        {
            return _dbContext.Entities.Where(p => p.ParentId == id);
        }
